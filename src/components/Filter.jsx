  ...
  return (
    <div className="flex gap-3 justify-center mt-14">
      {buttons.map((btn) => (
        ...
        duration-150
        active:scale-95
        hover:opacity-90 cursor-pointer px-4 py-2 rounded-full text-sm font-semibold ${filter === btn.value ? "bg-black text-white scale-105" : "bg-orange-100 text-orange-700 hover:bg-orange-200"} ${tasks.length === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          {btn.label}
        </button>
      ))}
